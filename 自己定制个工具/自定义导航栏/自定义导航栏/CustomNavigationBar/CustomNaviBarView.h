...
// 创建一个导航条按钮：无图片，只有文字。
+ (UIButton *)createCustomNaviBarBtnByTitle:(NSString *)strTitle target:(id)target action:(SEL)action;
...
+ (UIButton *)createLeftNavButtonWithTitle:(NSString *)title;
...
- (void)setLeftTextBtn:(UIButton *)btn;
...
//-(void)setTextRightButton:(BOOL)isText;
